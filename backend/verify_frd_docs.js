...
async function verifyFrdReformatting() {
        ...
        checks: [
          {
            item: "Events enabled",
            status: "Done",
            comment: "payment.captured, payment.failed",
          },
        ],
      },
    ],
  ...
  const mockEnrichmentData = {
    web_data: {
      description:
        "Verification Merchant is a test entity created for system validation.",
  ...
  try {
    ...
    const result = await frdGenerator.generateFRD(
      mockAuditResult,
      mockEnrichmentData,
      productType,
      diagramPath,
    ...
    expectedHeaders.forEach((header) => {
      if (content.includes(header)) {
        ...
        console.log(
          "Actual sample around header might be:",
    ...
    if (content.includes("<span")) {
      console.error(
        "❌ FAILURE: Restricted HTML tags (span) found in raw MD file."
      );
      process.exit(1);
    } else {
      console.log(
        "✅ SUCCESS: Minimal/Allowed HTML tags found in raw MD file."
      );
    }
    ...
    if (
      content.includes("https://razorpay.com/docs/payments/subscriptions/apis/")
    ) {
      console.log("✅ SUCCESS: Subscription APIs correctly linked.");
    } else if (content.includes("charge-at-will")) {
      console.error(
        "❌ FAILURE: Still showing Charge At Will docs for Subscriptions."
      );
      process.exit(1);
    } else {
